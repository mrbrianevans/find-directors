 ...
 */
export const respondToInvocation = (schema: Schema, logic: (args: Record<string, any>)=>any) => async (args: Record<string, any>): Promise<ReturnValue> => {
  let response: ReturnValue
  try{
    const valid = validate(args, schema)
    console.log(valid.errors, valid.instance, valid.schema)
    if(valid.valid){
      const result = await logic(args)
      response = {headers: { 'content-type':'application/json'}, body: JSON.stringify(result), statusCode: 200}
    }else{
      response = {headers: { 'content-type':'application/json'}, body: JSON.stringify({msg:'Bad request',errors: valid.errors.map(e=>e.message)}), statusCode: 400}
    }
  }catch (e) {
